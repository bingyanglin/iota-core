...
func Test_TransitionAndDestroyAccount(t *testing.T) {
	...
	ts.AssertAccountData(
		accounts.NewAccountData(genesisAccountOutput.AccountID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(iotago.BlockIssuanceCredits(123), 0)),
			accounts.WithOutputID(genesisAccount.ID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(oldGenesisOutputKey),
		), ts.Nodes()...)

	// validator node account.
	validatorAccountOutput := ts.AccountOutput("Genesis:2")
	ts.AssertAccountData(
		accounts.NewAccountData(node1.Validator.AccountData.ID,
	...
	block1 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block1", ts.DefaultWallet(), tx1, mock.WithSlotCommitment(genesisCommitment)))
	...
	ts.AssertAccountDiff(genesisAccountOutput.AccountID, block1Slot, &model.AccountDiff{
		...
		NewOutputID:            ts.DefaultWallet().OutputData("TX1:0").ID,
		PreviousOutputID:       genesisAccount.ID,
		BlockIssuerKeysRemoved: iotago.NewBlockIssuerKeys(),
		BlockIssuerKeysAdded:   iotago.NewBlockIssuerKeys(newGenesisOutputKey),
	}, false, ts.Nodes()...)

	ts.AssertAccountData(accounts.NewAccountData(genesisAccountOutput.AccountID,
		accounts.WithCredits(accounts.NewBlockIssuanceCredits(123, 0)),
		accounts.WithOutputID(ts.DefaultWallet().OutputData("TX1:0").ID),
		accounts.WithBlockIssuerKeys(oldGenesisOutputKey, newGenesisOutputKey),
		accounts.WithExpirySlot(newExpirySlot),
	), ts.Nodes()...)
...
func Test_StakeDelegateAndDelayedClaim(t *testing.T) {
	...
	tx1 := ts.DefaultWallet().CreateAccountFromInput(
		...
		mock.WithAccountMana(mock.MaxBlockManaCost(node1.Protocol.LatestAPI().ProtocolParameters())),
	)
	...
	newAccount := ts.DefaultWallet().AccountOutputData("TX1:0")
	...
	ts.AssertAccountData(accounts.NewAccountData(newAccountOutput.AccountID,
		...
		accounts.WithStakeEndEpoch(10),
		accounts.WithFixedCost(421),
		accounts.WithDelegationStake(0),
		accounts.WithValidatorStake(stakedAmount),
	), ts.Nodes()...)
	...
	latestParents = ts.CommitUntilSlot(block2Slot, block2.ID())
	delegatedAmount := ts.DefaultWallet().OutputData("TX1:1").Output.BaseTokenAmount()
	...
	ts.AssertAccountData(accounts.NewAccountData(newAccountOutput.AccountID,
		...
		accounts.WithStakeEndEpoch(10),
		accounts.WithFixedCost(421),
		accounts.WithDelegationStake(delegatedAmount),
		accounts.WithValidatorStake(stakedAmount),
	), ts.Nodes()...)
...
func Test_ImplicitAccounts(t *testing.T) {
	...
	implicitAccountOutput := newUserWallet.OutputData("TX1:0")
	...
	block2 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block2", newUserWallet, tx2, mock.WithStrongParents(latestParents...)))
	ts.CommitUntilSlot(block2Slot, block2.ID())

	fullAccountOutputID := newUserWallet.OutputData("TX2:0").ID
	...
	ts.AssertAccountData(accounts.NewAccountData(implicitAccountID,
		accounts.WithCredits(accounts.NewBlockIssuanceCredits(allotted-burned, block2Slot)),
		accounts.WithExpirySlot(iotago.MaxSlotIndex),
		accounts.WithOutputID(fullAccountOutputID),
		accounts.WithBlockIssuerKeys(fullAccountBlockIssuerKey),
	), ts.Nodes()...)
...
func Test_NegativeBIC_BlockIssuerLocked(t *testing.T) {
	...
	wallet1OutputID := ts.AccountOutput("Genesis:2").ID
	ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
		accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, 0)),
		accounts.WithOutputID(wallet1OutputID),
		accounts.WithExpirySlot(iotago.MaxSlotIndex),
		accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
	), ts.Nodes()...)

	// wallet 2 block issuer account.
	wallet2OutputID := ts.AccountOutput("Genesis:3").ID
	ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
		accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet2BIC, 0)),
		accounts.WithOutputID(wallet2OutputID),
		accounts.WithExpirySlot(iotago.MaxSlotIndex),
		accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
	), ts.Nodes()...)
	...
	// Issue one block from each of the two block-issuers - one will go negative and the other has enough BICs.
	{
		...
		ts.SetCurrentSlot(block1Slot)
		block11 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block1.1", wallet1, testPayload, mock.WithSlotCommitment(block1Commitment)))
		block12 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block1.2", wallet2, testPayload, mock.WithStrongParents(block11.ID()), mock.WithSlotCommitment(block1Commitment)))
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, block1Slot)),
			accounts.WithOutputID(wallet1OutputID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet2BIC, block1Slot)),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithOutputID(wallet2OutputID),
			accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
	...
	// the other has the block rejected in the PostSolidFilter as his account has negative BIC value.
	{
		...
		block21 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block2.1", wallet1, testPayload, mock.WithSlotCommitment(block2Commitment)))

		_, err := ts.IssueBasicBlockWithOptions("block2.2", wallet2, testPayload, mock.WithStrongParents(ts.BlockID("block2.1")), mock.WithSlotCommitment(block2Commitment))
		if !ierrors.Is(err, iotago.ErrAccountLocked) {
			t.Fatalf("expected block2.2 to be rejected with ErrAccountLocked, but got: %v", err)
		}
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, block2Slot)),
			accounts.WithOutputID(wallet1OutputID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
	...
	// The locked wallet 2 is preparing and signs the transaction, but it's issued by wallet 1 whose account is not locked.
	{
		...
		tx1 := wallet2.AllotManaFromInputs("TX1",
			iotago.Allotments{&iotago.Allotment{
				AccountID: wallet2.BlockIssuer.AccountData.ID,
		...
		block31 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block3.1", wallet1, tx1, mock.WithStrongParents(latestParents...), mock.WithSlotCommitment(block3Commitment)))
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, block31.ID().Slot())),
			accounts.WithOutputID(wallet1OutputID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet2BIC, block31.ID().Slot())),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithOutputID(wallet2OutputID),
			accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
	...
	// Issue block from the unlocked account of wallet 2 to make sure that it's actually unlocked.
	{
		...
		block4 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block4", wallet2, testPayload, mock.WithStrongParents(latestParents...), mock.WithSlotCommitment(block4Commitment)))
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, ts.BlockID("block3.1").Slot())),
			accounts.WithOutputID(wallet1OutputID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet2BIC, block4Slot)),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithOutputID(wallet2OutputID),
			accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
...
func Test_NegativeBIC_AccountOutput(t *testing.T) {
	...
	ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
		accounts.WithCredits(accounts.NewBlockIssuanceCredits(-1, 0)),
		accounts.WithOutputID(wallet1AccountOutput.ID),
		accounts.WithExpirySlot(iotago.MaxSlotIndex),
		accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
	), ts.Nodes()...)
	...
	ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
		accounts.WithCredits(accounts.NewBlockIssuanceCredits(iotago.MaxBlockIssuanceCredits/2, 0)),
		accounts.WithOutputID(wallet2AccountOutput.ID),
		accounts.WithExpirySlot(iotago.MaxSlotIndex),
		accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
	), ts.Nodes()...)
	...
	// The locked wallet 1 is preparing and signs the transaction, but it's issued by wallet 2 whose account is not locked.
	{
		...
		tx2 := wallet1.AllotManaFromInputs("TX2",
			iotago.Allotments{&iotago.Allotment{
				AccountID: wallet1.BlockIssuer.AccountData.ID,
				Mana:      iotago.Mana(allottedBIC),
			}}, "Genesis:0")

		block2Commitment := node1.Protocol.Engines.Main.Get().SyncManager.LatestCommitment().Commitment()
		// Wallet 2 whose account is not locked is issuing the block to unlock the account of wallet 1.
		block2 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block2", wallet2, tx2, mock.WithStrongParents(latestParents...), mock.WithSlotCommitment(block2Commitment)))
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet2BIC, block2Slot)),
			accounts.WithOutputID(wallet2AccountOutput.ID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
	...
	newExpirySlot = block3Slot + ts.API.ProtocolParameters().MaxCommittableAge()
	{
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, block3Slot)),
			accounts.WithOutputID(wallet1.AccountOutputData("TX3:0").ID),
			accounts.WithExpirySlot(newExpirySlot),
			accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()[0], newWallet1IssuerKey),
		), ts.Nodes()...)
	...
	// DESTROY WALLET 1 ACCOUNT
	{
		...
		block4 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block4", wallet2, tx4, mock.WithStrongParents(latestParents...), mock.WithSlotCommitment(block4Commitment)))
		latestParents = ts.CommitUntilSlot(block4Slot, block4.ID())

		// assert diff of the destroyed account.
		ts.AssertAccountDiff(wallet1.BlockIssuer.AccountData.ID, block4Slot, &model.AccountDiff{
			BICChange:              -iotago.BlockIssuanceCredits(wallet1BIC),
			PreviousUpdatedSlot:    block3Slot,
			NewExpirySlot:          0,
			PreviousExpirySlot:     newExpirySlot,
			NewOutputID:            iotago.EmptyOutputID,
			PreviousOutputID:       wallet1.OutputData("TX3:0").ID,
			BlockIssuerKeysAdded:   iotago.NewBlockIssuerKeys(),
...
func Test_NegativeBIC_AccountOwnedBasicOutputLocked(t *testing.T) {
	...
	block0 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block1", ts.DefaultWallet(), tx0, mock.WithSlotCommitment(genesisCommitment)))
	...
	// SEND A TRANSACTION TO AN ACCOUNT ADDRESS
	{
		...
		tx1 := wallet1.SendFundsToAccount(
			"TX1",
			wallet1.BlockIssuer.AccountData.ID,
			"TX0:0",
		)
		...
		// Wallet 2, which has non-negative BIC issues the block.
		block1 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block1", wallet2, tx1, mock.WithStrongParents(block0.ID()), mock.WithSlotCommitment(genesisCommitment)))
	...
	// but it's issued by wallet 2 whose account is not locked.
	{
		...
		// Wallet 2 whose account is not locked is issuing the block to unlock the account of wallet 1.
		block3 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block3", wallet2, tx3, mock.WithStrongParents(latestParents...), mock.WithSlotCommitment(block3Commitment)))
		...
		ts.AssertAccountData(accounts.NewAccountData(wallet2.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet2BIC, block3Slot)),
			accounts.WithOutputID(wallet2AccountOutput.ID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet2.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
	...
	// SPEND THE BASIC OUTPUT FROM AN ACCOUNT ADDRESS
	{
		...
		block4 := lo.PanicOnErr(ts.IssueBasicBlockWithOptions("block4", wallet1, tx4, mock.WithStrongParents(latestParents...), mock.WithSlotCommitment(block4Commitment)))
		...
		// The outputID of wallet1 and wallet2 account should remain the same as neither was successfully spent.
		ts.AssertAccountData(accounts.NewAccountData(wallet1.BlockIssuer.AccountData.ID,
			accounts.WithCredits(accounts.NewBlockIssuanceCredits(wallet1BIC, block4Slot)),
			accounts.WithOutputID(wallet1.OutputData("TX4:0").ID),
			accounts.WithExpirySlot(iotago.MaxSlotIndex),
			accounts.WithBlockIssuerKeys(wallet1.BlockIssuer.BlockIssuerKeys()...),
		), ts.Nodes()...)
