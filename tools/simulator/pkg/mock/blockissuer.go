...
func NewBlockIssuer(name string, keyManager *wallet.KeyManager, accountID iotago.AccountID, validator bool, opts ...options.Option[BlockIssuer]) *BlockIssuer {
...
func (i *BlockIssuer) CreateValidationBlock(ctx context.Context, issuerAccount wallet.Account, node *Node, opts ...options.Option[ValidationBlockParams]) (*blocks.Block, error) {
	...
	apiForBlock, _ := i.retrieveAPI(blockParams.BlockHeader, node)
	...
	if blockParams.BlockHeader.References == nil {
		references, _ := i.getReferencesValidationBlock(ctx, node, blockParams.BlockHeader.ParentsCount)

		blockParams.BlockHeader.References = references
	}

	i.setDefaultBlockParams(blockParams.BlockHeader, node)
	...
	if blockParams.ProtocolParametersHash == nil {
		protocolParametersHash, _ := apiForBlock.ProtocolParameters().Hash()
	...
	strongParents, _ := blockParams.BlockHeader.References[iotago.StrongParentType]
	...
	block, _ := blockBuilder.Build()

	// Make sure we only create syntactically valid blocks.
	modelBlock, _ := model.BlockFromBlock(block, serix.WithValidation())
...
func (i *BlockIssuer) CreateBasicBlock(ctx context.Context, node *Node, opts ...options.Option[BasicBlockParams]) (*blocks.Block, error) {
	...
	if blockParams.BlockHeader.References == nil {
		references, _ := i.getReferencesBasicBlock(ctx, node, blockParams.BlockHeader.ParentsCount)
		blockParams.BlockHeader.References = references
	}
