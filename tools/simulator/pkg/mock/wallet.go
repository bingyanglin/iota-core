...
func NewWallet(name string, node *Node, keyManager ...*wallet.KeyManager) *Wallet {
	...
	return &Wallet{
		...
		keyManager:   km,
		BlockIssuer:  NewBlockIssuer(name, km, iotago.EmptyAccountID, false),
	}
}

func (w *Wallet) SetBlockIssuer(accountID iotago.AccountID) {
	w.BlockIssuer = NewBlockIssuer(w.Name, w.keyManager, accountID, false)
}
